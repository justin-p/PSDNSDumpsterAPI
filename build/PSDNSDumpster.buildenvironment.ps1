...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        Encoding = 'utf8'
        ModuleToBuild = 'PSDNSDumpster'
        ModuleVersion = '0.0.1'
        ModuleWebsite = 'https://github.com/justin-p/PSDNSDumpster'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) Justin Perdok. All rights reserved."
        ModuleLicenseURI = 'https://github.com/justin-p/PSDNSDumpster/LICENSE.md'
        ModuleTags = 'DNSDumpster, API' -split ','
        ModuleAuthor = 'Justin Perdok'
        ModuleDescription = 'Unofficial DNSDumpster API'
        ...
        # You should NOT enter this key here but rather manually enter it in the PSDNSDumpster.buildenvironment.json file with: set-buildenvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "PSDNSDumpster.buildenvironment.json"
